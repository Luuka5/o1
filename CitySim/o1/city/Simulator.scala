...
class Simulator:
  ...
  def findDemographic(demographic: Demographic): Vector[GridPos] = {
    // TODO
    Vector() ++ this.cityMap.allPositions
      .filter(pos => this.cityMap.matches(pos, demographic))
  }
  ...
  def dissatisfiedResidents: Vector[GridPos] = {
    ...
    this.cityMap.allPositions.filter(pos => {
      val positions = Vector(
        pos.neighbor(CompassDir.East),
      )

      val occupied = positions.count(pos => {
        this.cityMap.matches(pos, )
      })
      // TODO: Fix this
      occupied * 2 > dirs.length
    })
  }
